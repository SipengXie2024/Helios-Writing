...
# Find common blocks
common_blocks = set(seq_df['block_number']) & set(optim_df['block_number'])
...
optim_filtered = optim_df[optim_df['block_number'].isin(common_blocks)].sort_values('block_number')
merged = pd.merge(seq_filtered, optim_filtered, on='block_number', suffixes=('_seq', '_optim'))

# Calculate speedups
speedups = merged['elapsed_time_ms_seq'] / merged['elapsed_time_ms_optim']

# Print statistics
print(f"\nOnline Mode (No Filtering) Speedup Statistics:")
...
ax.set_title('Online Mode Speedup Distribution (No Filtering)', fontsize=16, color='black', fontweight='bold', pad=20)
...
plt.tight_layout()
plt.savefig('online_speedup_distribution.pdf', dpi=300, bbox_inches='tight', facecolor='white')
plt.savefig('online_speedup_distribution.png', dpi=300, bbox_inches='tight', facecolor='white')
print(f"\nSaved online_speedup_distribution.pdf and online_speedup_distribution.png")
plt.close()
