...
\subsubsection{Replay Mode Performance (RQ2 \& RQ3)}

Replay mode assumes a precomputed transaction plan and represents the best-case scenario for archive nodes. On the 5{,}000 evaluated blocks, it achieves a median speedup of 6.60$\times$ over Revm Native, with the 75th and 90th percentiles at 13.88$\times$ and 21.43$\times$ respectively (Figure~\ref{fig:replay_speedup_dist}). Only 5.4\% of blocks are slower than baseline.

These results indicate that once transaction paths have been traced and optimized, Helios can substantially accelerate repeated re-execution of historical blocks, making it well-suited for archive node synchronization and analytic workloads.

\subsubsection{Online Mode Effectiveness (RQ2 \& RQ3)}

Online mode must predict execution paths using only the current CallSig and cached statistics. We evaluate it on 1{,}000 blocks immediately following the warmup range.


Without filtering (all paths cached), Online mode reaches a median speedup of 4.56$\times$, with 75th and 90th percentiles at 7.12$\times$ and 9.85$\times$; 8.8\% of blocks are slower than baseline (Figure~\ref{fig:online_no_filter}). Applying frequency $\geq 10$ filtering reduces the median speedup to 2.05$\times$ and the 75th percentile to 4.47$\times$, while the 90th percentile remains high at 9.01$\times$ (Figure~\ref{fig:online_filtered}). Thus, frequency-based pruning trades median performance for a substantial drop in storage, but preserves strong acceleration for the hottest blocks.

The remaining gap between Online and Replay performance stems from three factors: (i) Top-1 prediction covers only 58.0\% of executions under frequency filtering, so many transactions still fall back to native execution; (ii) Helios uses transaction-level rollback on mispredictions, discarding partially optimized work instead of switching paths mid-execution; and (iii) prediction and cache lookup add fixed per-transaction overhead that becomes visible when optimized paths execute in a few microseconds. We discuss potential extensions such as multi-path caching and finer-grained fallback in \S\ref{sec:discussion}.