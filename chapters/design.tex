...
First, it replaces the EVM stack with a direct-mapped virtual register file indexed by LSN. Each SsaGraph node writes its result to a dedicated slot, enabling consumers to access operands via $O(1)$ array indexing. This design eliminates \texttt{DUP}/\texttt{SWAP} and other stack manipulation overhead.
